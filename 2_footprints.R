...
FP_plant <- t(t(MP) * Y_plant) ########  Error: cannot allocate vector of size 4.6 Gb
# FP_plant <- sum(FP_plant)
# FP_plant <- sum(colSums(MP) * Y_plant) ########  Error: cannot allocate vector of size 4.6 Gb


# create output matrix
supply_chain_FP <- data.frame(Scenario = rep("SQ", 4),
                              chain_type = c("plant_based", "plant_based", "animal_based", "animal_based"),
                              flow = c("waste", "cont", "waste", "cont"))


# BIG CALCULATIONS--
FP_tmp <- sum(FP_plant * waste$harvest_production) / 100               # waste shares are given as percentage (8% = 0,08)
FP_plant_sum <- sum(FP_plant)

supply_chain_FP$harvest_production <- c(FP_tmp, FP_plant_sum, NA, NA) # column has 4 rows # FAO production data reflect 'cont' flow

supply_chain_FP$storage_transport <- c(FP_tmp, FP_plant_sum - FP_tmp, NA, NA) # column has 4 rows
...
step.calculator <- function(waste_step, FP){                      # waste_step is column in waste, eg. waste$harvest_production?
  ...
  FP_tmp <- FP  - (FP* waste_step)                  
  return(list(FP_step,FP_tmp))
}
